...
@Component({
  selector: 'app-p-interview',
  templateUrl: './p-interview.page.html',
  styleUrls: ['./p-interview.page.scss'],
})
export class PInterviewPage implements ViewWillEnter {
  public Melamar: any = [];
  public Cabang: any = [];
  search: any;
  ...
  async getMelamar() {
    try {
      this._apiService.getMelamar('Interview').subscribe(
        (res: any) => {
          this.Melamar = res;
        },
        (error: any) => {
  ...
  async Search() {
    try {
      this._apiService.SearchMelamar2('Interview', this.search).subscribe(
        (res: any) => {
          this.Melamar = res;
        },
        (error: any) => {
